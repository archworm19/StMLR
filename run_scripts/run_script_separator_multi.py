...
def remake_separator(S, rc):
    num_models = S.MLRC.num_models
    num_state = S.MLRC.num_state
    submodels_per_state = S.MLRC.submodels_per_state
    output_cells = S.MLRC.output_cells
    output_classes = S.MLRC.output_classes   
    ...
    MLRC = separator.MultiLogRecComp(num_models, num_state, submodels_per_state, xdim, output_cells, output_classes, \
        l1_mask, l2_state_mask)    

    # reassign MLR in separator:
    S.MLR = MLRC
